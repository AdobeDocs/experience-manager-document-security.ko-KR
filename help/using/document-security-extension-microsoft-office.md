...
description: Microsoft&reg; Office용 Document Security Extension을 사용하면 Microsoft&reg; Office 파일에 사전 정의된 기밀 유지 설정을 적용할 수 있습니다.
...
Microsoft® Office용 Document Security Extension은 Adobe Experience Manager Forms용 LiveCycle Rights Management 및 Document Security를 향상시킵니다. Office 파일을 보호하고 인증된 사용자가 설정된 기밀 유지 설정에 따라 정책으로 보호된 파일에 액세스할 수 있도록 합니다.
...
Document Security 관리자와 사용자는 정책을 수립합니다. 관리자는 모든 인가된 사용자가 사용할 수 있는 조직 정책을 수립합니다. 관리자 또는 정책 세트 코디네이터는 사용자 하위 집합이 사용할 수 있는 *정책 세트*&#x200B;라는 정책 그룹을 생성할 수도 있습니다. 사용자는 자신만 사용할 수 있는 자체 정책을 만들 수 있습니다. 관리자, 정책 세트 코디네이터 및 사용자는 Document Security 웹 페이지를 사용하여 정책을 수립합니다.

정책은 Document Security에 저장되지만 Word, Excel 또는 PowerPoint를 통해 파일에 적용할 수 있습니다. 파일에 정책을 적용하면 파일에 포함된 정보는 정책에 지정된 기밀 유지 설정에 의해 보호됩니다. 정책으로 보호된 파일을 배포하면 권한이 있는 수신자만 해당 콘텐츠에 액세스할 수 있습니다.

정책을 사용하여 파일을 보호하면 배포 후에도 해당 파일을 지속적으로 제어할 수 있습니다. 이벤트를 감사하여 수신자가 파일을 사용하는 시기와 방법을 추적할 수 있습니다. 정책을 변경하고, 사용자가 파일에 계속 액세스하지 못하도록 하고, 파일에 첨부된 정책을 변경할 수도 있습니다.
...
정책이 파일에 적용된 후 언제든지 정책의 기밀 유지 설정을 변경할 수 있습니다. 권한이 부여된 사용자가 파일을 받은 후 해당 사용자를 추가 또는 제거하거나 사용자의 권한을 변경할 수 있습니다. 파일에 적용되는 정책은 변경될 수 있습니다. 또한 파일 사본이 있는 사용자가 더 이상 파일을 열 수 없도록 파일에 대한 액세스 권한이 취소될 수 있습니다.
...
사용자가 정책으로 보호된 Word, Excel 및 PowerPoint 파일을 열고 사용하려면 정책에 사용자를 수신자로 포함해야 합니다. 또는 익명 액세스를 허용해야 합니다. 또한 사용자에게 Microsoft® Office용 Document Security Extension이 설치되어 있어야 합니다. Microsoft® Office용 Document Security Extension이 없는 사람에게 정책으로 보호된 파일을 제공하려면 소프트웨어 사본을 제공할 수 있습니다. 또는 웹 사이트에서 다운로드하는 방법을 알려 줄 수 있습니다. 설치 관리자가 없는 경우 [다운로드 페이지](https://experienceleague.adobe.com/en/docs/experience-manager-document-security/using/download-installer)에서 다운로드할 수 있습니다.
...
* 관리자 또는 정책을 적용한 사람이 파일에 대한 액세스를 취소하거나 정책을 변경할 때까지.
  ...
  정책이 오프라인 액세스를 허용하는 경우 사용자는 지정된 오프라인 임대 기간 동안 인터넷 또는 네트워크 연결 없이 정책으로 보호된 파일을 열 수 있습니다. 오프라인 임대 기간이 종료되면 사용자는 온라인으로 전환하고 Document Security와 동기화하여 새 임대 기간을 시작해야 합니다.

  정책이 오프라인 액세스를 허용하는 경우 사용자는 지정된 오프라인 임대 기간 동안 인터넷 또는 네트워크 연결 없이 정책으로 보호된 파일을 열 수 있습니다. 새 파일 열기 시도와 같은 이벤트도 원본 파일과 동일하게 감사되고 기록됩니다.
...
입찰을 수락한 후 제조업체는 정책을 업데이트하여 로컬에서 인쇄하고 복사하고 변경 사항을 저장할 수 있는 권한을 낙찰자에게 부여합니다. 제조업체는 실패한 입찰자도 제거하여 파일을 열 수 있는 권한을 취소합니다.
...
<table >
 <tbody>
  ...
  <tr>
   ...
   <td><p><a href="https://experienceleague.adobe.com/en/docs/experience-manager-65/content/forms/administrator-help/get-started/configure-general-aem-forms-settings">관리자 도움말</a>을 참조하거나 Document Security 관리 페이지에서 페이지의 오른쪽 상단에 있는 도움말 링크를 클릭하십시오.</p> </td>
  </tr>
  <tr>
